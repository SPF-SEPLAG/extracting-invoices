    ...
    def tokenize_labeled(self):
        ...
        for labeled_file in self.output_labeled_folder.glob("labeled_*.json"):
            with open(labeled_file, "r", encoding="utf-8") as f:
                ...
                encoding = self.tokenizer(words, boxes=boxes, word_labels=labels, padding="max_length", truncation=True, return_tensors="pt", max_length=514)
                ...
                # for token_id, token, box, attention_mask, label in zip(token_ids, tokens, token_boxes, attention_masks, token_labels):
                #     print(f"ID: {token_id.item():6} | Token: {token:15} | BBox: {box.tolist()} | Masks: {attention_mask.item()} | Label: {label.item()}")
